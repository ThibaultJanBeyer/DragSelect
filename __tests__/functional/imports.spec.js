...
const teardown = async () => {
  await page.evaluate(() => {
    window.importScripts.forEach(iScript => document.body.removeChild(iScript))
    window.ds = null
    window.callback = []
...
describe('Imports', () => {
  ...
  beforeAll(async () => new Promise((resolve, rejects) => {
    try {
  ...
  describe('AMD Module', () => {
    const setup = async (uri = '../../dist/DragSelect') => {
      await page.evaluate(() => {
        window.dsScript = document.createElement('script')
        window.dsScript.setAttribute('src', `http://requirejs.org/docs/release/2.3.6/comments/require.js`)
        document.body.appendChild(window.dsScript)
        window.importScripts.push(window.dsScript)
      })
  ...
  describe('ESM Module', () => {
    ...
    it('when importing the script as module, it should be available', async () => {
      await goToPage(`http://localhost:${port}/__tests__/functional/imports.html`)
      await setup()
      await test()
      //await wait(20000)
      await teardown()
