...
> Disclaimer: By hooking into the selection you’re modifying internal behavior. Don’t expect support for any misbehaving caused by hooking in. We will try our best to not introduce breaking changes the overrideable methods but it’s best if you double check each time before updating the library.
...
If you want to change the selectioning during the selection process (not before, not after) you can use the exposed method `filterSelected`. We’ll explain how here.
...
That’s it, you’ve successfully added a custom filter to the selection logic. Now go celebrate that you’ve just learned something new! 🎉
