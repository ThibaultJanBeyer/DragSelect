...
export type DSPublishMappings<E extends DSInputElement> = 
  & DSSettingsPublish<E>
  & DSAreaPublish<E>
  & DSKeyStorePublish
...
export default class PubSub<E extends DSInputElement> {
  subscribers: {[K in keyof DSPublishMappings<E>]?: DSCallback<DSPublishMappings<E>[K]>[]} = {}
  DS: DragSelect<E>
   ...
   */
  subscribe = <K extends keyof DSPublishMappings<E>>(eventName: K, callback: DSCallback<DSPublishMappings<E>[K]>) => {
    if(!Array.isArray(this.subscribers[eventName])) this.subscribers[eventName] = []
    const subscribers = this.subscribers[eventName] as DSCallback<DSPublishMappings<E>[K]>[]
   ...
   */
  unsubscribe = <K extends keyof DSPublishMappings<E>>(eventName: K, callback?: DSCallback<DSPublishMappings<E>[K]>, id?: number) => {
   ...
   */
  publish = <K extends keyof DSPublishMappings<E>>(eventName: K|K[], data: DSPublishMappings<E>[K]) => {
    if (Array.isArray(eventName))
      eventName.forEach((name) => this._publish(name, data))
    else this._publish(eventName, data)
  }

  private _publish = <K extends keyof DSPublishMappings<E>>(eventName: K, data: DSPublishMappings<E>[K]) => {
    const subscribers: DSCallback<DSPublishMappings<E>[K]>[] = this.subscribers[eventName] ?? []
  ...
  private _handlePublish = <K extends keyof DSPublishMappings<E>>(subscribers: DSCallback<DSPublishMappings<E>[K]>[], data: DSPublishMappings<E>[K]) => {
  ...
  _handlePrePublish = <K extends keyof DSPublishMappings<E>>(subscribers: DSCallback<DSPublishMappings<E>[K]>[], data: DSPublishMappings<E>[K]) => {
