...
export type DSPublicPublishEventNames<E extends DSInputElement> = keyof DSPublicPublish<E>
...
type UsedPublishMappings<E extends DSInputElement> = Pick<DSPublishMappings<E>, UsedPublishNames>

type Condition<T, E extends DSInputElement> = (data: T, DS: DragSelect<E>) => T | null

type MappingObj<E extends DSInputElement, K extends keyof UsedPublishMappings<E>> = {
...
const endExtraData = <E extends DSInputElement>(data: UsedPublishMappings<E>["Interaction:end"], DS: DragSelect<E>) => {
...
export const subscriberAliases = <E extends DSInputElement>({ PS, DS }: { PS: PubSub<E>; DS: DragSelect<E> }) => {
  ...
  for (const [sub_name, sub_pubs] of Object.entries(mapping) as [keyof UsedPublishMappings<E>, DSMappings<E>[keyof UsedPublishMappings<E>]][])
    addSubscribers({ sub_name, sub_pubs, DS, PS })
}

const addSubscribers = <E extends DSInputElement, K extends keyof UsedPublishMappings<E>>(
  { sub_name, DS, PS, sub_pubs }:
  { sub_name: K, sub_pubs: DSMappings<E>[K], DS: DragSelect<E>, PS: PubSub<E> }
) => {
...
const publish = <E extends DSInputElement, K extends keyof UsedPublishMappings<E>>(
  { sub_pub, data, DS }:
  { sub_pub: MappingObj<E, K>, data: DSPublishMappings<E>[K], DS: DragSelect<E> }
) => {
